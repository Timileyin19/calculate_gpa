...
function enterResult(e) {
    ...
    for (let i = 1; i <= (parseInt(numOfCourses.value)); i++) {
        // create div element 
        let inputDiv, inputField, selectUnitDiv, selectUnitField, selectGradeDiv, selectGradeField; 
        inputDiv = document.createElement('div'); 
        // Add class to the div
        inputDiv.className = 'form-group mr-3 mb-3';
     ...
     // create div element  
      ...
      // create div element  
         ...
         // Add class to the div
         selectGradeDiv.className = 'form-group mb-3';
         // create the select element that will be atttached to this form group 
         selectGradeField = document.createElement('select'); 
         selectGradeField.className = 'form-control';
         ...
         selectGradeDiv.append(document.createElement('br'));
...
function calculateGPA() {
    ...
    let grades = [], units = [], gradeUnit = [];
    for(let i = 1; i <= (parseInt(numOfCourses.value)); i++) {
        grades.push(document.querySelector('#grade'+i).value);
        units.push(parseInt(document.querySelector('#unit'+i).value));
        gradeUnit.push((document.querySelector('#grade'+i).value) * (document.querySelector('#unit'+i).value))
    }
...
function remark(gpa) {
    ...
    else if (gpa >= 3.5 && gpa < 4.5) { return 'Second Class Upper'}
    else if (gpa >= 2.5 && gpa < 3.5 ) { return 'Second Class Lower'}
    else if (gpa >= 1.5 && gpa < 2.5 ) { return 'Third Class'}
    else if (gpa >= 1.0 && gpa < 1.5 ) {return 'Pass'} 
    else { return 'Fail'} 
}